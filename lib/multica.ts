...
export async function reportErrorToMultica(title: string, details: string) {
  ...
  try {
    ...
    // Mention the review agent, Atlas, and Codex in a comment to trigger auto-fix/collaboration
    const mentionReview = `[${REVIEW_AGENT_NAME}](mention://agent/${REVIEW_AGENT_ID})`;
    const mentionAtlas = `[Atlas](mention://agent/${ATLAS_AGENT_ID})`;
    const mentionCodex = `[${CODEX_AGENT_NAME}](mention://agent/${CODEX_AGENT_ID})`;
    
    const commentPayload = {
      content: `Cc: ${mentionReview}, ${mentionAtlas}, ${mentionCodex}\n\n이 서버 에러 이슈를 확인하고 코드를 수정해주거나 분석을 진행해주세요.`,
    };
